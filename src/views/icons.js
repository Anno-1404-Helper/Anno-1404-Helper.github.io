...
export function renderIcons(ctx) {
  ctx.render(iconsTemplate());
}

const iconsTemplate = () => html`<h1>Icons and Consumption Rates</h1>
  <section class="main">
    <div class="clear">
      <table class="left">
...
const row = (_, y) => html`<tr>
  ${new Array(22).fill(0).map((_, x) => col(x, y))}
</tr>`;

const col = (x, y) =>
  html`<td @click=${onClick} data-x=${x} data-y=${y}>
    <span class="sub">${x},${y}</span>${iconTemplate(46, x, y, [])}
  </td>`;
