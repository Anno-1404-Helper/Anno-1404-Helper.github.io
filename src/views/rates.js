...
export function renderRates(ctx) {
  const rates = ctx.settings.production;

  ctx.render(ratesTemplate(Object.entries(rates)));
}

const ratesTemplate = (rates) => html`<h1>Production and Consumption Rates</h1>
  <section class="main">
    <table>
      <thead>
        <tr>
...
const ratesRow = (
  [type, { output, input_A, rate_A, input_B, rate_B }],
  settings
) => html` <tr>
  <td>${icon(type)}</td>
  ...
  <td>
    <span class="label">${getRate(output, input_A, rate_A, settings)}</span>
  </td>
  <td>${input_B && icon(input_B)}</td>
  <td><span class="label">${rate_B}</span></td>
  <td>
    <span class="label">${getRate(output, input_B, rate_B, settings)}</span>
  </td>
</tr>`;
